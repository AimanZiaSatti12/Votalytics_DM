...
#st.sidebar.image("C:/Users/MA/OneDrive/Pictures/voting.gif")
# Load data
def load_data():
    senate_file = None
    presidential_file = None

    uploaded_files = st.file_uploader("Upload CSV files", type="csv", accept_multiple_files=True)

    if uploaded_files is not None:
        for uploaded_file in uploaded_files:
            if "senate" in uploaded_file.name.lower():
                senate_file = uploaded_file
            elif "president" in uploaded_file.name.lower():
                presidential_file = uploaded_file

    if senate_file is not None and presidential_file is not None:
        senate_df = pd.read_csv(senate_file, encoding='latin-1')
        presidential_df = pd.read_csv(presidential_file, encoding='latin-1')
...
if user_menu == "Overall Analysis":
   # st.image("C:/Users/MA/Downloads/Votalytics.png")
...
if user_menu=="Sentiment Analysis":
    @st.cache_data
    def load_data(trump_file, biden_file):
        Trump_reviews = pd.read_csv(trump_file, index_col=0)
        Biden_reviews = pd.read_csv(biden_file, index_col=0)
        return Trump_reviews, Biden_reviews


    unique_key_trump = f"trump_{time.time()}"
    unique_key_biden = f"biden_{time.time()}"

    uploaded_trump_file = st.file_uploader("Upload Trump CSV file", type="csv", key=unique_key_trump)
    uploaded_biden_file = st.file_uploader("Upload Biden CSV file", type="csv", key=unique_key_biden)

    if uploaded_trump_file and uploaded_biden_file:
        Trump_reviews, Biden_reviews = load_data(uploaded_trump_file, uploaded_biden_file)
...
# Sample data loading (assuming senate_df and presidential_df are already loaded)
# senate_df = pd.read_csv('senate_data.csv')
# presidential_df = pd.read_csv('presidential_data.csv')
...
if user_menu == 'Clustering':
    ...
    if option == 'Senate':
        ...
        if len(selected_features) < 3:
            st.warning('Please select at least three features for 3D clustering.')
        else:
            # Preprocess data
            ...
            for cluster in range(k):
                ...
                ax.scatter(cluster_data[selected_features[0]], cluster_data[selected_features[1]], cluster_data[selected_features[2]], label=f'Cluster {cluster}')
        ...
        if len(selected_features_presidential) < 3:
            st.warning('Please select at least three features for 3D clustering.')
        else:
            # Preprocess data
            ...
            for cluster in range(k):
                ...
                ax.scatter(cluster_data[selected_features_presidential[0]], cluster_data[selected_features_presidential[1]], cluster_data[selected_features_presidential[2]], label=f'Cluster {cluster}')
            ax.set_xlabel(selected_features_presidential[0])
            ax.set_ylabel(selected_features_presidential[1])
            ax.set_zlabel(selected_features_presidential[2])
